...
function applyEmpresa(e){
  ...
  document
    .querySelector("#pageDescription")
    ?.setAttribute(
      "content",
      `${nome} — demonstração personalizada de delivery digital.`
    );

  document.querySelector("#benefitBrand").textContent =
    nome.toUpperCase();

  document.querySelector("#footerBrand").textContent =
    nome.toUpperCase();

  document.querySelector("#contactPhone").textContent =
    `💬 ${e.whatsapp || "WhatsApp não informado"}`;

  document.querySelector("#contactInstagram").textContent =
    `📷 ${instagramHandle(e.instagram_url)}`;

  document.querySelector("#contactLocation").textContent =
    `📍 ${
      [e.cidade,e.estado]
        .filter(Boolean)
  ...
  const delivery =
    document.querySelector("#deliveryBike");
  ...
  /*
   * Em uma demo comercial nenhum botão
   * deve enviar pedido real.
   */
  [
    "#heroWhatsapp",
    "#contactWhatsapp",
...
function demoOnly(ev){
  ...
  alert(
    "Ambiente demonstrativo WAP. Nenhum pedido será enviado."
  );
}
...
function renderFilters(){
  ...
  box.innerHTML =
    ...
    cats.map(c =>
      `<button data-filter="${slugify(c)}">${c}</button>`
    ).join("");
}
...
function renderPizzas(filter="todas"){
  ...
  const grid =
    document.querySelector("#pizzaGrid");

  const list =
    filter === "todas"
      ? pizzas
      : pizzas.filter(
          p => slugify(p.categoria) === filter
        );
  ...
  grid.innerHTML = list.map(p => `
    ...
    <article class="card">
      ...
      <img
        src="${p.imagem_url || imageFallback(p.nome)}"
        alt="${p.nome}"
        onerror="this.onerror=null;this.src='${imageFallback(p.nome)}'"
      >
      ...
      <div class="card-body">
        ...
        <p>
          ${p.descricao || "Item do cardápio demonstrativo."}
        </p>
        ...
        <div class="card-bottom">
          ...
          <span class="price">
            ...
            ${
              p.preco_publico
                ? '<small class="public-price"> preço público</small>'
                : ''
            }
          </span>
...
document.querySelector("#continueOrder").onclick =
  closeCart;
...
document
  .querySelector("#orderType")
  .addEventListener("change", e => {

    document.querySelector("#address").style.display =
...
document
  ...
  .addEventListener("click", () => {
    ...
    if(!cart.length){
      ...
      alert(
        "Adicione pelo menos um item ao carrinho para testar o fluxo."
      );
    ...
    alert(
      "Simulação concluída. Este ambiente é uma demonstração WAP e nenhum pedido foi enviado."
    );
  });
...
async function supabaseRest(path, params={}){
  ...
  if(!res.ok){
    ...
    throw new Error(
      `Supabase REST ${res.status}: ${detail}`
    );
  }
...
async function boot(){
  ...
  console.info(
    "WAP Demo V2: carregando cliente",
    slug
  );
  ...
  try{
    ...
    console.info(
      "WAP Demo V2: carregado com sucesso",
      {
        empresa:e.nome,
        produtos:pizzas.length
      }
    );
     ...
     * a demonstração continua funcionando.
     */
